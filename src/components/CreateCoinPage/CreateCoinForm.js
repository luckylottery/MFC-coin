...
export const CreateCoinForm = ({selectedCrypto}) => {
  ...
  return (
    ...
    <Form {...layout} form={form} name="control-hooks" onFinish={onFinish} onValuesChange={handleFormValuesChange}>
      ...
      <Form.Item
        name="initialSupply"
        label={<label className="font-bold text-sm">Initial
          Supply</label>}
        className="grid grid-cols-1 sm:grid-cols-formItem p-4 text-blue-2"
        rules={[{
          required: true,
      ...
      <Form.Item name="decimals"
                 label={<label className="font-bold text-sm">Decimals <span
      ...
      <Form.Item name="advancedSettings" label={<label className="font-bold text-sm">Advanced Settings</label>}
                 className="px-4 text-blue-2" initialValue={advancedSettings}
                 getValueFromEvent={(data) => data}>
        <Switch checked={advancedSettings} className="mt-4" handleDiameter={20} uncheckedIcon={null} checkedIcon={null}
                onColor="#7280bc" offColor="#e4e9ff"
      ...
      {advancedSettings ?
        <Form.Item name="createOwner" rules={[{
          whitespace: true, message: <label className="text-xs text-red-700">Please input your
            owner</label>
        }]} label={<label className="font-bold text-blue-2 text-sm">Create Owner</label>}
                   className="grid grid-cols-1 sm:grid-cols-formItem p-4">
          <Input type="text"
                 className="w-full p-0.5 rounded focus:outline-none"/>
        </Form.Item> : null}

      <Form.Item name="otherConfig" label={<label className="font-bold text-sm">Other Config</label>}
        ...
        <Checkbox.Group style={{width: '100%'}} className="grid grid-cols-2">
          <Checkbox value="deflation" className="whitespace-nowrap p-2">
            <span className="ml-4">Deflation</span></Checkbox>
        </Checkbox.Group>
      </Form.Item>
      {
        deflation ? <>
          <Form.Item name="transactionTax"
                     label={<label className="font-bold text-sm">Transaction Tax:</label>}
                     className="grid grid-cols-1 sm:grid-cols-formItem p-4 text-blue-2"
                     rules={[{
                       required: true,
                       message: <label className="text-xs text-red-700">Input required</label>
                     }, {
                       whitespace: true, message: <label className="text-xs text-red-700">Input required</label>
                     }]}>
            <Input
              className="w-full col-start-2 col-end-5 p-0.5 rounded focus:outline-none"/>
          </Form.Item>
          <Form.Item name="taxReceiveAddress"
                     label={<label className="font-bold text-sm">Tax Receive address:</label>}
                     className="grid grid-cols-1 sm:grid-cols-formItem p-4 text-blue-2"
                     rules={[{
                       required: true,
                       message: <label className="text-xs text-red-700">Input required</label>
          ...
          <Form.Item name="holdersRewardFee" label={<label className="font-bold text-sm">Holders reward fee:</label>}
          ...
          <Form.Item name="automaticLp" label={<label className="font-bold text-sm">Automatic LP:</label>}
          ...
          {automaticLp ? (
            <>
              <Form.Item name="automaticLpInput"
              ...
              <Form.Item name="transactionTaxSelect"
                         label={<label className="font-bold text-sm">Transaction Tax:</label>}
                         className="grid grid-cols-1 sm:grid-cols-formItem p-4 text-blue-2"
                         rules={[{
                           required: true,
