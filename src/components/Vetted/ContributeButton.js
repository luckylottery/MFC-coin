    ...
    if (walletAddress === "") {
      ...
      try {
        ...
        const tx = await callWithGasPrice(
          idoContract,
          "_UserDepositPhaseOne",
          [],
          {
            value: ethers.utils.parseEther(
              String(Number(contributeValue) + 0.001)
            ),
          }
        );
      } catch (e) {
    ...
    <div className="mt-4 w-full flex flex-col gap-3 items-center">
      {walletAddress !== "" && (
        <input
          ...
          } border-2 text-blue-3 border-blue-3  font-bold p-2 text-right rounded-sm w-full outline-none`}
      ...
      <button
        ...
        className={`${
          isLoading ? "opacity-50 cursor-wait" : ""
        } flex justify-center gap-1.5 border-blue-3 transition duration-300 border-2 text-blue-3  font-bold text-md rounded-sm py-2 w-full`}
